...
Fastify provides several [hooks](../fastify-docs/Hooks-Fastify.md) useful for a variety of situations. One of them is the [`onReady`](../fastify-docs/Hooks-Fastify.md#onready) hook, which is useful for executing tasks _right before_ the server starts accepting new requests. There isn't, though, a direct mechanism to handle scenarios in which you'd like the server to start accepting **specific** requests and denying all others, at least up to some point.
...
1.  the mechanism for authenticating with the provider [decorating](../fastify-docs/Decorators.md) the `fastify` object with the authentication key (`magicKey` from here onward)
...
What we'll do to improve this setup overall is create a [`Plugin`](../fastify-docs/Plugins-Fastify.md) that'll be solely responsible for making sure we both:
...
The keywords here were [Decorators](../fastify-docs/Decorators.md), [Hooks](../fastify-docs/Hooks-Fastify.md), and [Plugins](../fastify-docs/Plugins-Fastify.md). Combining what Fastify has to offer can lead to very ingenious and creative solutions to a wide variety of problems. Let's be creative! :)
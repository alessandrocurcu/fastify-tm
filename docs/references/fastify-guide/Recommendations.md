...
-   Prefer static or simple parametric routes on hot paths. RegExp routes are expensive, and routes with many parameters can also hurt router performance. See [Routes - Url building](../fastify-docs/Routes-Fastify.md#url-building).
-   Use route constraints carefully. Version constraints can degrade router performance, and asynchronous custom constraints should be treated as a last resort. See [Routes - Constraints](../fastify-docs/Routes-Fastify.md#constraints).
-   Prefer Fastify plugins/hooks over generic middleware when possible. Fastify's middleware adapters work, but native integrations are typically better for performance-sensitive paths. See [Middleware](../fastify-docs/Middleware.md).
-   Define response schemas to speed up JSON serialization. See [Getting Started - Serialize your data](./Getting-Started.md#serialize-data).
-   Keep Ajv `allErrors` disabled by default. Enable it only when detailed validation feedback is needed (for example, form-heavy APIs), and avoid it on latency-sensitive endpoints. When `allErrors: true` is enabled, validation can do more work per request and make denial-of-service attacks easier on untrusted inputs. See also:
    -   [Validation and Serialization - Validator Compiler](../fastify-docs/Validation-and-Serialization.md#schema-validator)

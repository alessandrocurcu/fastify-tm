...
Your routes, your utilities, and so on are all plugins. To add a new plugin, whatever its functionality may be, in Fastify you have a nice and unique API: [`register`](../fastify-docs/Plugins-Fastify.md).
...
Fastify offers you a more elegant and comfortable way to do this, _decorators_. Creating a decorator is extremely easy, just use the [`decorate`](../fastify-docs/Decorators.md) API:
...
We have seen how to extend server functionality and how to handle the encapsulation system, but what if you need to add a function that must be executed whenever the server "[emits](../fastify-docs/Lifecycle-Fastify.md)" an event?
...
So what can you do to avoid this annoying issue? Yes, you are right, use a [hook](../fastify-docs/Hooks-Fastify.md)!
...
An alternative approach is to make use of the [onRoute hook](../fastify-docs/Hooks-Fastify.md#onroute) to customize application routes dynamically from inside the plugin. Every time a new route is registered, you can read and modify the route options. For example, based on a [route config option](../fastify-docs/Routes-Fastify.md#routes-options):

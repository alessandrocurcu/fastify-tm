---
title: "Helmet.js"
site: "helmetjs.github.io"
source: "https://helmet.js.org/"
domain: "helmetjs.github.io"
language: "en"
word_count: 1829
---
...
The `Content-Security-Policy` header mitigates a large number of attacks, such as [cross-site scripting](https://developer.mozilla.org/en-US/docs/Glossary/Cross-site_scripting). See [MDN’s introductory article on Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP).
...
`action` is a string that specifies which directive to use—either `DENY` or `SAMEORIGIN`. (A legacy directive, `ALLOW-FROM`, is not supported by Helmet. [Read more here.](http://helmetjs.github.io/faq/x-frame-options-allow-from-directive/)) It defaults to `SAMEORIGIN`.

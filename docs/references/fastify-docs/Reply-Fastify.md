...
Invokes the custom not found handler. Note that it will only call `preHandler` hook specified in [`setNotFoundHandler`](./Server-Fastify.md#set-not-found-handler).
...
See special note about error handling for streams in [`setErrorHandler`](./Server-Fastify.md#seterrorhandler).
...
If you want to customize error handling, check out [`setErrorHandler`](./Server-Fastify.md#seterrorhandler) API.
...
Beware that calling `reply.send(error)` in your custom error handler will send the error to the default error handler. Check out the [Reply Lifecycle](./Lifecycle-Fastify.md#reply-lifecycle) for more information.

The not found errors generated by the router will use the [`setNotFoundHandler`](./Server-Fastify.md#setnotfoundhandler)
...
The type of the sent payload (after serialization and going through any [`onSend` hooks](./Hooks-Fastify.md#onsend)) must be one of the following types, otherwise, an error will be thrown:
...
If you want to know more please review [Routes#async-await](./Routes-Fastify.md#async-await).

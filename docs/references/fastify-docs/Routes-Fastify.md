...
-   `method`: currently it supports `GET`, `HEAD`, `TRACE`, `DELETE`, `OPTIONS`, `PATCH`, `PUT` and `POST`. To accept more methods, the [`addHttpMethod`](./Server-Fastify.md#addHttpMethod) must be used. It could also be an array of methods.
    
-   `url`: the path of the URL to match this route (alias: `path`).
    
-   `schema`: an object containing the schemas for the request and response. They need to be in [JSON Schema](https://json-schema.org/) format, check [here](./Validation-and-Serialization.md) for more info.
...
-   `exposeHeadRoute`: creates a sibling `HEAD` route for any `GET` routes. Defaults to the value of [`exposeHeadRoutes`](./Server-Fastify.md#exposeHeadRoutes) instance option. If you want a custom `HEAD` handler without disabling this option, make sure to define it before the `GET` route.
...
-   `onRequest(request, reply, done)`: a [function](./Hooks-Fastify.md#onrequest) called as soon as a request is received, it could also be an array of functions.
    
-   `preParsing(request, reply, payload, done)`: a [function](./Hooks-Fastify.md#preparsing) called before parsing the request, it could also be an array of functions.
    
-   `preValidation(request, reply, done)`: a [function](./Hooks-Fastify.md#prevalidation) called after the shared `preValidation` hooks, useful if you need to perform authentication at route level for example, it could also be an array of functions.
    
-   `preHandler(request, reply, done)`: a [function](./Hooks-Fastify.md#prehandler) called just before the request handler, it could also be an array of functions.
    
-   `preSerialization(request, reply, payload, done)`: a [function](./Hooks-Fastify.md#preserialization) called just before the serialization, it could also be an array of functions.
    
-   `onSend(request, reply, payload, done)`: a [function](./Hooks-Fastify.md#route-hooks) called right before a response is sent, it could also be an array of functions.
    
-   `onResponse(request, reply, done)`: a [function](./Hooks-Fastify.md#onresponse) called when a response has been sent, so you will not be able to send more data to the client. It could also be an array of functions.
    
-   `onTimeout(request, reply, done)`: a [function](./Hooks-Fastify.md#ontimeout) called when a request is timed out and the HTTP socket has been hung up.
    
-   `onError(request, reply, error, done)`: a [function](./Hooks-Fastify.md#onerror) called when an Error is thrown or sent to the client by the route handler.
    
-   `handler(request, reply)`: the function that will handle this request. The [Fastify server](./Server-Fastify.md) will be bound to `this` when the handler is called. Note: using an arrow function will break the binding of `this`.
    
-   `errorHandler(error, request, reply)`: a custom error handler for the scope of the request. Overrides the default error global handler, and anything set by [`setErrorHandler`](./Server-Fastify.md#seterrorhandler), for requests to the route. To access the default handler, you can access `instance.errorHandler`. Note that this will point to fastify's default `errorHandler` only if a plugin hasn't overridden it already.
    
-   `childLoggerFactory(logger, binding, opts, rawReq)`: a custom factory function that will be called to produce a child logger instance for every request. See [`childLoggerFactory`](./Server-Fastify.md#childloggerfactory) for more info. Overrides the default logger factory, and anything set by [`setChildLoggerFactory`](./Server-Fastify.md#setchildloggerfactory), for requests to the route. To access the default factory, you can access `instance.childLoggerFactory`. Note that this will point to Fastify's default `childLoggerFactory` only if a plugin hasn't overridden it already.
...
-   `serializerCompiler({ { schema, method, url, httpStatus, contentType } })`: function that builds schemas for response serialization. See the [Validation and Serialization](./Validation-and-Serialization.md#schema-serializer) documentation.
...
-   `handlerTimeout`: maximum number of milliseconds for the route's full lifecycle. Overrides the server-level [`handlerTimeout`](./Server-Fastify.md#factory-handler-timeout). Must be a positive integer. When the timeout fires, `request.signal` is aborted and a 503 error is sent through the error handler (which can be customized per-route).
...
-   `prefixTrailingSlash`: string used to determine how to handle passing `/` as a route with a prefix.
    ...
    Note: this option does not override `ignoreTrailingSlash` in [Server](./Server-Fastify.md) configuration.
    
-   `request` is defined in [Request](./Request-Fastify.md).
    
-   `reply` is defined in [Reply](./Reply-Fastify.md).
    

> ℹ️ Note: The documentation for `onRequest`, `preParsing`, `preValidation`, `preHandler`, `preSerialization`, `onSend`, and `onResponse` is detailed in [Hooks](./Hooks-Fastify.md). To send a response before the request is handled by the `handler`, see [Respond to a request from a hook](./Hooks-Fastify.md#respond-to-a-request-from-a-hook).
...
Be aware that setting `logLevel` at the plugin level also affects [`setNotFoundHandler`](./Server-Fastify.md#setnotfoundhandler) and [`setErrorHandler`](./Server-Fastify.md#seterrorhandler).
...
Custom version matching logic can be defined through the [`constraints`](./Server-Fastify.md#constraints) configuration when creating a Fastify server instance.

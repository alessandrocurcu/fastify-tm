...
Fastify can be extended with plugins, which can be a set of routes, a server [decorator](./Decorators.md), or other functionality. Use the `register` API to add one or more plugins.
...
The [Getting Started](../fastify-guide/Getting-Started.md#your-first-plugin) guide includes an example of using this API:
...
-   [`logLevel`](./Routes-Fastify.md#custom-log-level)
-   [`logSerializers`](./Routes-Fastify.md#custom-log-serializer)
...
If an option with the key `prefix` and a `string` value is passed, Fastify will use it to prefix all the routes inside the register. For more info, check [here](./Routes-Fastify.md#route-prefixing).

Be aware that if routes are wrapped with [`fastify-plugin`](https://github.com/fastify/fastify-plugin), this option will not work (see the [workaround](./Routes-Fastify.md#fastify-plugin)).
...
If `register` is used only to extend server functionality with [`decorate`](./Decorators.md), tell Fastify not to create a new scope. Otherwise, changes will not be accessible in the upper scope.
